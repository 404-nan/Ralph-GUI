...
- Task が多いときだけ `MaxIntegration` を自動で増やし、active task の幅に反映する
- Task を panel 上で最優先へ上げたり後ろへ回したりできる
- README / PRD / issue / 要件メモを panel に貼り付けて preview し、そのまま Task 化できる
...
初見だとここが一番迷いやすいので、先に結論を書くと `RALPH_TASK_CATALOG_FILE` には「長い仕様書」ではなく「着手順に並んだ task catalog JSON」を置くのが一番使いやすいです。
...
- 大きな README や要件メモは、そのまま置かずに 5-15 件くらいの story に分解する

GitHub README や docs の URL しかなくても問題ありません。リンクを読める ChatGPT / Codex に「この URL を読んで `prd.json` を作って」と渡せば、そのまま task catalog 化できます。コピペ用テンプレートと詳しい書式は [docs/task-catalog.md](./docs/task-catalog.md) にまとめました。
...
- `回答待ち`: pending question ごとにその場で回答できる
- `Task ワークスペース`: 追加、編集、完了チェック、未完了への戻し、優先順位の前後移動
- `仕様書から一括追加`: README / PRD / issue / メモを貼って preview し、そのまま Task 化する
- `人の確認待ち`: 未回答の質問と回答履歴
...
Discord から操作も行う場合は `RALPH_DISCORD_ALLOWED_USER_IDS` または `RALPH_DISCORD_DM_USER_ID` のどちらかを設定してください。どちらも空のときは通知専用モードになります。
`RALPH_DISCORD_GUILD_ID` を設定すると、操作を受け付ける guild を 1 つに制限できます。
...
- Task catalog / PRD ガイド: [docs/task-catalog.md](./docs/task-catalog.md)
