...
RalphLoop は `RALPH_TASK_CATALOG_FILE` で指定した JSON を起点に Task board を作れます。
...
```text
この GitHub README / docs を読んで、この repo 用の RalphLoop task catalog JSON を作ってください。
...
```text
この README / URL を読んで、RalphLoop の task catalog JSON だけをコードブロックで返してください。
...
```text
次のメモを RalphLoop 用の task catalog JSON に整理してください。
- LP を作る
- 問い合わせフォームを付ける
- 管理画面で問い合わせ一覧を見る
- メール通知も欲しい
