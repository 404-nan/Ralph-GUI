...
import { Alert, Card, EmptyState, Metric, ToneBadge } from './PanelPrimitives.tsx';
import {
  laneLabel,
  laneTone,
  priorityLabel,
...
function TaskChip(props: { task: TaskBoardItem; onSelect: (taskId: string) => void }) {
  return (
    <button
      type="button"
    ...
    >
      ...
      {props.task.summary && <div className="mt-1 text-sm text-slate-400">{props.task.summary}</div>}
    ...
    <div className="space-y-6">
      {dashboard.setupDiagnostics.summary.error > 0 && (
        <Alert
          ...
          message="Setup タブで path / prompt / agent command を修正してください。"
        />
      )}
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <Metric label="Run State" value={runStateLabel(dashboard.status.runState)} detail={dashboard.runReason} />
        <Metric
          label="Current Task"
          value={dashboard.currentTask ? dashboard.currentTask.id : 'None'}
          detail={dashboard.currentTask?.title ?? 'No current task'}
        />
        <Metric
          label="Next Up"
          value={String(dashboard.nextTasks.length)}
          detail={`${dashboard.blockedTasks.length} blocked / ${dashboard.doneTasks.length} done`}
        />
        <Metric
          label="Completion"
          value={dashboard.runReport.completion.status.replace('_', ' ')}
          detail={dashboard.runReport.completion.message}
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <Card
          title="Current Task"
          subtitle="Single-active execution model. Only one task is current at a time."
          ...
          {dashboard.currentTask ? (
            <div className="space-y-5">
              ...
              <div>
                ...
                {dashboard.currentTask.summary && <div className="mt-2 text-sm leading-6 text-slate-300">{dashboard.currentTask.summary}</div>}
              </div>
              ...
              {dashboard.currentTask.acceptanceCriteria.length > 0 ? (
                <div>
                  ...
                  <ul className="mt-3 space-y-2">
                    {dashboard.currentTask.acceptanceCriteria.map((criterion) => (
                      <li key={criterion} className="flex items-start gap-3 text-sm text-slate-300">
                ...
                <Alert tone="warning" title="Acceptance criteria が空です" message="DONE signal だけで完了扱いにしないため、criteria を追加してください。" />
              )}
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <Card title="Next Up Queue" subtitle="Operator-visible queue after the current task.">
            {dashboard.nextTasks.length > 0 ? (
              <div className="space-y-3">
                {dashboard.nextTasks.map((task) => <TaskChip key={task.id} task={task} onSelect={props.onSelectTask} />)}
              </div>
            ) : (
              <EmptyState title="Next task は空です" detail="次に実行する仕事がまだ queue されていません。" />
            )}
          </Card>

          <Card title="Pending Decisions" subtitle="Human input blocking confident execution.">
            {dashboard.pendingDecisions.length > 0 ? (
              ...
              <EmptyState title="Pending decision はありません" detail="今は operator judgement を待っていません。" />
            )}
          </Card>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr_1fr]">
        <Card title="Blockers" subtitle="Why the mission is stopped or at risk.">
          {dashboard.blockedTasks.length > 0 || dashboard.runReport.completion.pendingItems.length > 0 || dashboard.blockers.length > 0 ? (
            ...
            <EmptyState title="Blocker は記録されていません" detail="run が止まる理由は現在観測されていません。" />
          )}
        </Card>

        <Card title="Evidence & Artifacts" subtitle="Completion quality is evidence-based, not DONE-only.">
          <div className="space-y-4">
            ...
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                <FileDiff className="h-4 w-4" /> Changed Files
              </div>
              {dashboard.runReport.changedFiles.length > 0 ? (
                <div className="space-y-2">
                  {dashboard.runReport.changedFiles.map((file) => <div key={file} className="rounded-2xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-sm text-slate-300">{file}</div>)}
                </div>
              ) : (
            ...
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                <FlaskConical className="h-4 w-4" /> Test Summary
              </div>
              {dashboard.runReport.testSummary.length > 0 ? (
                <div className="space-y-2">
                  {dashboard.runReport.testSummary.map((line) => <div key={line} className="rounded-2xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-sm text-slate-300">{line}</div>)}
                </div>
              ) : (
            ...
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                <FileCheck2 className="h-4 w-4" /> Artifacts
              </div>
        ...
        <Card title="Recent Outputs" subtitle="Why the run is moving or why it is stuck.">
